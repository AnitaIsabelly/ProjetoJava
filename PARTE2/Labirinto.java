...
public class Labirinto
{
  ...
  */
  ...
  /**
  Construtor da classe Labirinto, instanciando tambem os seus atributos. O metodo recebe a matriz do labirinto por parametro e instancia a variavel caminho, adjacente e possibilidades.
  ...
  */
  ...
  {
    if(labirinto==null) throw new Exception("Matriz ausente");
  ...
  @return Retorna o caminho percorrido.
  */
  ...
  @param matriz Matriz criada da classe CriaLabirinto.
  @throws Exception se houver espaco nas paredes do labirinto (paredes faltando), 
                    ...
                    ou se nao houver matriz definida.
  */
  public void encontrarEntradaSaida(char[][] matriz) throws Exception
  {
    if(matriz == null) throw new Exception("Matriz ausente");
    int contadorE=0;
    int contadorS=0;
    int parede=0;
    //VERIFICA LINHA I = X
    try{
      for(int i=0; i<matriz.length; i++){
        ...
        if (matriz[i][matriz[0].length-1]=='E')
        {
          this.atual = new Coordenada ((byte)i, (byte)(matriz[0].length-1));
          contadorE++;
        }
        else if (matriz[i][matriz[0].length-1]=='S')
        {
          this.saida = new Coordenada ((byte)i, (byte)(matriz[0].length-1));
          contadorS++;
        }
        if(matriz[i][0]==' ' || matriz[i][matriz[0].length-1]==' ')
          parede=1;
      }
      //VERIFICA COLUNA J = Y
      for(int j=0; j<matriz[0].length; j++){
        ...
        else if(matriz[0][j]=='S')
        {
          this.saida = new Coordenada ((byte)0, (byte)j);
          contadorS++;
        }
        if(matriz[matriz.length-1][j]=='E')
        {
          this.atual = new Coordenada ((byte)(matriz.length-1), (byte)j);
          contadorE++;
        }
        else if(matriz[this.matriz.length-1][j]=='S')
        {
          this.saida = new Coordenada ((byte)(matriz.length-1), (byte)j);
          contadorS++;
        }
        if(matriz[0][j]==' ' || matriz[matriz.length-1][j]==' ')
          parede=1;
      }
    }
    catch(Exception erro){}
    if(parede==1)
      throw new Exception("Labirinto com paredes faltando");
    if(this.atual==null)
      throw new Exception("Entrada invalida");
    if(this.saida==null)
      throw new Exception("Saida invalida");
    if(contadorE>1)
      throw new Exception("Multiplas entradas");
    if(contadorS>1)
      throw new Exception("Multiplas saidas");
  }
